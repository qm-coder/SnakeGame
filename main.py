...
THEMES = [
    ...
    # ---- 新增三大主题 ----
    {
        "name": "Nezha Mode",  # 哪吒模式
        "bg": (139, 26, 26), "grid": (165, 42, 42), "text": (255, 215, 0),  # 暗红背景（风火轮/火莲），金色文字
        "p_head": (255, 218, 185), "p_body": (255, 69, 0),  # 哪吒：肤色头，火红色身体（混天绫）
        "e_head": (135, 206, 235), "e_body": (70, 130, 180)  # 敖丙（龙）：冰蓝色系
    },
    {
        "name": "Pleasant Goat",  # 喜羊羊模式
        "bg": (152, 251, 152), "grid": (143, 188, 143), "text": (0, 100, 0),  # 青青草原绿
        "p_head": (255, 250, 250), "p_body": (135, 206, 250),  # 喜羊羊：白头（羊毛），亮蓝身（脖套/鞋子）
        "e_head": (80, 80, 80), "e_body": (105, 105, 105)  # 灰太狼：深灰头，浅灰身
    },
    {
        "name": "Ultraman Mode",  # 奥特曼模式
        "bg": (25, 25, 112), "grid": (40, 40, 130), "text": (0, 255, 255),  # 宇宙星空深蓝，青色发光文字
        "p_head": (220, 220, 220), "p_body": (220, 20, 20),  # 奥特曼：银色金属头，红色身体
        "e_head": (139, 69, 19), "e_body": (85, 107, 47)  # 小怪兽：棕褐色头，军绿色身体（哥莫拉/巴尔坦星人）
    }
]
...
class Game:
    def __init__(self):
        pygame.init()
        pygame.display.set_caption("Snake V8.0 (8 Themes Ultimate)")
        self.screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
        self.clock = pygame.time.Clock()

        self.font = pygame.font.SysFont("Arial", 24, bold=True)
        self.big_font = pygame.font.SysFont("Arial", 48, bold=True)
        self.cartoon_font = pygame.font.SysFont("comicsansms", 40, bold=True)
        if not self.cartoon_font: self.cartoon_font = self.big_font
        ...
        # 首页按钮
        self.btn_easy = pygame.Rect(cx - btn_w // 2, cy + 20, btn_w, btn_h)
        self.btn_normal = pygame.Rect(cx - btn_w // 2, cy + 85, btn_w, btn_h)
        self.btn_hard = pygame.Rect(cx - btn_w // 2, cy + 150, btn_w, btn_h)
        # 首页进入主题选择的按钮
        self.btn_theme_menu = pygame.Rect(cx - btn_w // 2, cy + 230, btn_w, btn_h)
        ...
        self.state = "MENU"  # States: MENU, PLAYING, PAUSED, GAMEOVER, THEME_SELECT
    ...
    def save_data(self):
        with open(DATA_FILE, 'w') as f: json.dump(self.stats, f)
    ...
    def reset_game(self):
        ...
        self.foods = []
        self.golden_foods = []
        for _ in range(5): self.add_food()
